...
class Gyro {
  ...
  public:
    ...
    void GyroCaluculate() {
      ...
      Alltime_gyro = GetGyro() - first_gyro;

      if (gyro_now > 179.99)
        gyro_now -= 359;
      else if (gyro_now < -179.99)
        gyro_now += 359;

      if (Alltime_gyro > 179.99)
        Alltime_gyro -= 359;
      else if (Alltime_gyro < -179.99)
        Alltime_gyro += 359;

      gyro_cal += gyro_delta;

      if ((gyro_bef >= 90) && ((0 > gyro_now) && (gyro_now >= -179.99)))
        gyro_bef -= 359;
      else if ((gyro_bef <= -90) && ((0 < gyro_now) && (gyro_now <= 179.99)))
        gyro_bef += 359;
